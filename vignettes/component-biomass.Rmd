...
***Important:*** Do not use the `jenkinsRatio()` on tree biomass
that has been calculated via `treeCalc()` using either the `ung` or `lambert` 
method. This will lead to an inaccurate calculation as `ung` and `lambert` do 
not include root biomass in their estimation.
